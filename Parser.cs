        ...
        internal List<Cache> LoadGPXWaypoints()
        {
            ...
            var waypoints = (from waypoint in XDoc.Descendants(gpx + "wpt")
                             select new Cache
                             {
                                 ...
                                 Difficulty = float.Parse(waypoint.Element(gs + "cache").Element(gs + "difficulty").Value).ToString("N1"),
                                 Terrain = float.Parse(waypoint.Element(gs + "cache").Element(gs + "terrain").Value).ToString("N1"),
